...
@ManagedBean
@ViewScoped
public class StudentGroupSearchBean extends BaseManagedBean implements Serializable {
	
	private static final Logger logger = LogManager.getLogger(StudentGroupSearchBean.class);
    ...
    private StudentGroup currentObject;
    
    private GenericPagination<StudentGroup> pagination;
	...
	public void search() {
		...
		try {
			...
			List<StudentGroup> list = serviceStudentGroupBL
					.finder(null, filterName, sortType);
			
    		pagination = new GenericPagination<StudentGroup>(list, PAGINATION_SIZE);
	...
	public void initCurrentObject() {
		currentObject = new StudentGroup();
	...
	private void _create() throws Exception {
		...
		try {
			...
			serviceStudentGroupBL.create(currentObject, super.getLoggedUserId());
	...
	private void _update() throws Exception {
		...
		try {
			...
			serviceStudentGroupBL.update(currentObject, super.getLoggedUserId());
			
			for (StudentGroup center : pagination.getObjectList()) {
	...
	public void deleteCurrentObject() throws Exception {
		...
		try {
			...
			serviceStudentGroupBL.delete(currentObject.getId());
